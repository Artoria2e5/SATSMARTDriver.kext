...
#define Endian16_Swap(value) \
    ((((UInt16)((value) &0x00FF)) << 8) | (((UInt16)((value) &0xFF00)) >> 8))
...
/////////////
static void
hexdump16(UInt16* data, int start, int len)
{
    int i, j;
...
static void
trimcpy(char* target, const char* source, int maxlen)
{
    const char* p;
    char*       q;
    int         i;
...
void
swapbytes(IOBufferMemoryDescriptor* buffer)
{
    int     i;
...
int
countzeros(IOBufferMemoryDescriptor* buffer)
{
    int    i, count = 0;
...
UInt8
checksum(IOBufferMemoryDescriptor* buffer)
{
    int    i;
    UInt8  sum = 0;
    UInt8* ptr = (UInt8*) buffer->getBytesNoCopy();
...
bool
fi_dungeon_driver_IOSatDriver::init(OSDictionary* dict)
{
...
#ifdef DEBUG
void
fi_dungeon_driver_IOSatDriver::free(void)
{
    DEBUG_LOG("%s[%p]::%s\n", getClassName(), this, __FUNCTION__);
    super::free();
}
...
IOService*
fi_dungeon_driver_IOSatDriver::probe(IOService* provider,
    SInt32*                                     score)
{
...
OSObject*
fi_dungeon_driver_IOSatDriver::getParentProperty(const char* key)
{
...
void
fi_dungeon_driver_IOSatDriver::parseProperties()
{
    OSString*  value;
    OSBoolean* flag;
...
bool
fi_dungeon_driver_IOSatDriver::start(IOService* provider)
{
    DEBUG_LOG("%s[%p]::%s\n", getClassName(), this, __FUNCTION__);
...
void
fi_dungeon_driver_IOSatDriver::stop(IOService* provider)
{
    DEBUG_LOG("%s[%p]::%s\n", getClassName(), this, __FUNCTION__);
...
IOReturn
fi_dungeon_driver_IOSatDriver::setProperties(OSObject* properties)
{
...
IOReturn
fi_dungeon_driver_IOSatDriver::sendSMARTCommand(IOSatCommand* command)
{
    DEBUG_LOG("%s[%p]::%s\n", getClassName(), this, __FUNCTION__);
    ...
    DEBUG_LOG("buffer %p, bytecount %d, features %x, opcode %x, timeout %d, seccnt %x, secnum %x, cyllo %x, cylhi %x, dev %x, cmd %x, flags %x\n", cmd->getBuffer(), (int) cmd->getByteCount(), cmd->getErrorReg(), cmd->getOpcode(), (int) cmd->getTimeoutMS(), cmd->getSectorCount(), cmd->getSectorNumber(), cmd->getCylLo(), cmd->getCylHi(), cmd->getDevice_Head(), cmd->getStatus(), cmd->getFlags());
    ...
    if ((serviceResponse == kSCSIServiceResponse_TASK_COMPLETE) && (GetTaskStatus(request) == kSCSITaskStatus_GOOD || (fPermissive && (GetTaskStatus(request) == kSCSITaskStatus_CHECK_CONDITION))))
    // TODO senseData.VALID_RESPONSE_CODE == 0x72 && senseData.SENSE_KEY == 5) ?
    {
        ...
        ERROR_LOG("REQUEST: [ %02x %02x %02x %02x %02x %02x %02x ]"
            ...
            cmd->getErrorReg(), cmd->getSectorCount(), cmd->getSectorNumber(), cmd->getCylLo(), cmd->getCylHi(), cmd->getDevice_Head(), cmd->getStatus(), cmd->getFlags(), cmd->getOpcode(), (int) cmd->getTimeoutMS(), direction, count, protocol, (int) cmd->getByteCount());
...
fi_dungeon_driver_IOSatDriver::JMicron_get_registers(UInt16 address, UInt8* ptr, UInt16 length)
{
    ...
    if ((serviceResponse == kSCSIServiceResponse_TASK_COMPLETE) && (GetTaskStatus(request) == kSCSITaskStatus_GOOD)) {
...
void
fi_dungeon_driver_IOSatDriver::LogAutoSenseData(SCSITaskIdentifier request)
{
    SCSI_Sense_Data senseData;
    GetAutoSenseData(request, &senseData, sizeof(senseData));
...
void
fi_dungeon_driver_IOSatDriver::TerminateDeviceSupport(void)
{
    DEBUG_LOG("%s[%p]::%s\n", getClassName(), this, __FUNCTION__);
    ...
    int direction, int transferCount)
{
    ...
    case kSCSIDataTransfer_NoDataTransfer:
        ...
        break;
    default:
        __Require(("bad scsi direction", 0), ErrorExit);
    }
