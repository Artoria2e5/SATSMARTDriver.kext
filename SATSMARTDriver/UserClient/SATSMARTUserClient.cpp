...
IOExternalMethod SATSMARTUserClient::sMethods[kIOATASMARTMethodCount] = {
    { // Method #0 EnableDisableOperations
        0, (IOMethod) &SATSMARTUserClient::EnableDisableOperations, kIOUCScalarIScalarO, 1, 0 },
    { // Method #1 EnableDisableAutoSave
        0, (IOMethod) &SATSMARTUserClient::EnableDisableAutoSave, kIOUCScalarIScalarO, 1, 0 },
    { // Method #2 ReturnStatus
        0, (IOMethod) &SATSMARTUserClient::ReturnStatus, kIOUCScalarIScalarO, 0, 1 },
    { // Method #3 ExecuteOfflineImmediate
        0, (IOMethod) &SATSMARTUserClient::ExecuteOfflineImmediate, kIOUCScalarIScalarO, 1, 0 },
    { // Method #4 ReadData
        0, (IOMethod) &SATSMARTUserClient::ReadData, kIOUCStructIStructO, 0, sizeof(ATASMARTData) },
    { // Method #5 ReadDataThresholds
        0, (IOMethod) &SATSMARTUserClient::ReadDataThresholds, kIOUCStructIStructO, 0, sizeof(ATASMARTDataThresholds) },
    { // Method #6 ReadLogAtAddress
        0, (IOMethod) &SATSMARTUserClient::ReadLogAtAddress, kIOUCStructIStructO, sizeof(ATASMARTReadLogStruct),
        kIOUCVariableStructureSize },
    { // Method #7 WriteLogAtAddress
        0, (IOMethod) &SATSMARTUserClient::WriteLogAtAddress, kIOUCStructIStructO, sizeof(ATASMARTWriteLogStruct), 0 },
    { // Method #8 GetIdentifyData
        0, (IOMethod) &SATSMARTUserClient::GetIdentifyData, kIOUCStructIStructO, 0, kATADefaultSectorSize }
...
bool
SATSMARTUserClient::initWithTask(task_t owningTask, void* securityToken, UInt32 type)
{
...
SATSMARTUserClient::ReturnStatus(UInt32* exceededCondition)
{
    ...
    if (status == kIOReturnSuccess) {
        ...
        if ((lbaMid == kSMARTReturnStatusValidLoValue) && (lbaHigh == kSMARTReturnStatusValidHiValue)) {
            *exceededCondition = 1;
        }
...
IOReturn
SATSMARTUserClient::ReadData(UInt32* dataOut, IOByteCount* outputSize)
{
    IOReturn            status  = kIOReturnSuccess;
    IOSATCommand*       command = NULL;
...
IOReturn
SATSMARTUserClient::ReadDataThresholds(UInt32* dataOut, IOByteCount* outputSize)
{
    IOReturn            status  = kIOReturnSuccess;
    IOSATCommand*       command = NULL;
    IOMemoryDescriptor* buffer  = NULL;
...
IOReturn
SATSMARTUserClient::ReadLogAtAddress(
    ATASMARTReadLogStruct* structIn, void* structOut, IOByteCount inStructSize, IOByteCount* outStructSize)
{
    IOReturn            status  = kIOReturnSuccess;
    IOSATCommand*       command = NULL;
    IOMemoryDescriptor* buffer  = NULL;
...
IOReturn
SATSMARTUserClient::WriteLogAtAddress(ATASMARTWriteLogStruct* writeLogData, UInt32 inStructSize)
{
    IOReturn            status  = kIOReturnSuccess;
    IOSATCommand*       command = NULL;
    IOMemoryDescriptor* buffer  = NULL;
    ...
    if (inStructSize != sizeof(ATASMARTWriteLogStruct) || writeLogData->numSectors > 16
        || writeLogData->data_length > kSATMaxDataSize) {
        return kIOReturnBadArgument;
    }
    ...
    buffer = IOMemoryDescriptor::withAddressRange(
        writeLogData->data_pointer, writeLogData->data_length, kIODirectionOut, fTask);
...
IOReturn
SATSMARTUserClient::GetIdentifyData(UInt32* dataOut, IOByteCount* outputSize)
{
    IOReturn            status  = kIOReturnSuccess;
    IOSATCommand*       command = NULL;
    IOMemoryDescriptor* buffer  = NULL;
...
SATSMARTUserClient::SendSMARTCommand(IOSATCommand* command)
{
    ...
    if (status == kIOReturnSuccess) {
        fCommandGate->runAction((IOCommandGate::Action) &SATSMARTUserClient::sWaitForCommand, (void*) command);
...
IOReturn
SATSMARTUserClient::sWaitForCommand(void* userClient, IOSATCommand* command)
{
